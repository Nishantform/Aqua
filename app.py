        ...
        with engine.connect() as conn:
            with conn.begin():
                ...
                if 'water_usage_history' in tables and 'water_sources' in tables:
                    try:
                        usage = pd.read_sql("""SELECT wu.*, ws.source_name, ws.source_type, ws.state, ws.district 
                                               FROM water_usage_history wu LEFT JOIN water_sources ws ON wu.source_id = ws.source_id""", conn)
                    except:
                        usage = pd.DataFrame()
                else:
                    usage = pd.DataFrame()
...
if not sources.empty:
    ...
    if 'capacity_percent' in sources.columns:
        sources['health_score'] = (sources['capacity_percent'].fillna(50) * 0.5 + 
                                   (100 - sources['age'].clip(0, 100).fillna(50)) * 0.3 + 20).clip(0, 100)
        sources['risk_level'] = pd.cut(sources['capacity_percent'], bins=[0, 30, 60, 100], 
                                        labels=['Critical', 'Moderate', 'Good'], include_lowest=True)
    else:
        sources['risk_level'] = 'Unknown'

if not groundwater.empty and 'avg_depth_meters' in groundwater.columns:
    groundwater['stress_level'] = pd.cut(groundwater['avg_depth_meters'], bins=[0, 20, 40, 100], 
                                          labels=['Low', 'Moderate', 'High'])

if not rainfall.empty and 'rainfall_cm' in rainfall.columns:
    rainfall['rainfall_category'] = pd.cut(rainfall['rainfall_cm'], bins=[0, 50, 150, 300, float('inf')], 
...
with tab2:
    ...
    style_map = {"Esri Satellite": "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", 
    ...
    if not filtered_sources.empty and 'latitude' in filtered_sources.columns:
        ...
        for _, source in swc.iterrows():
            ...
            marker = folium.CircleMarker(location=[source['latitude'], source['longitude']], radius=marker_size + (3 if cap < 30 else 0), 
                                        color=color, fill=True, fillOpacity=0.7, popup=folium.Popup(popup_html, max_width=300), 
                                        tooltip=f"{source.get('source_name')} - {cap:.0f}%")
            marker.add_to(mc)
    ...
    if show_stations and not filtered_stations.empty and 'latitude' in filtered_stations.columns:
        for _, station in filtered_stations[filtered_stations['latitude'].notna() & filtered_stations['longitude'].notna()].iterrows():
            ...
            folium.Marker(location=[station['latitude'], station['longitude']], icon=folium.Icon(color=station_color, icon='info-sign'), 
                         popup=folium.Popup(station_popup, max_width=300), tooltip=f"Station: {station.get('station_name')}").add_to(m)
    ...
    if not water_quality.empty:
        ...
        with col1:
            if 'ph_level' in water_quality.columns:
                fig = px.histogram(water_quality, x='ph_level', nbins=20, title="pH Distribution", 
        ...
        with col2:
            if 'dissolved_oxygen_mg_l' in water_quality.columns:
                fig = px.box(water_quality, y='dissolved_oxygen_mg_l', title="Dissolved Oxygen Distribution", 
        ...
        available_cols = [c for c in display_cols if c in water_quality.columns]
        if available_cols:
            display_df = water_quality[available_cols].copy()
...
with tab6:
    ...
    if table_choice == "Water Sources":
        ...
        if not display_df.empty:
            cols = [c for c in ['source_id', 'source_name', 'source_type', 'capacity_percent', 'max_capacity_mcm', 
            ...
            st.download_button("📥 Download CSV", display_df.to_csv(index=False).encode('utf-8'), 
                              f"water_sources_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
    
    elif table_choice == "Monitoring Stations":
        display_df = filtered_stations if not filtered_stations.empty else stations
        if not display_df.empty:
            cols = [c for c in ['station_id', 'station_name', 'state_name', 'district_name', 'latitude', 'longitude', 
                               'ph_level', 'dissolved_oxygen_mg_l', 'turbidity_ntu', 'status'] if c in display_df.columns]
            st.dataframe(display_df[cols] if cols else display_df, use_container_width=True, hide_index=True)
            st.download_button("📥 Download CSV", display_df.to_csv(index=False).encode('utf-8'), 
                              f"stations_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
        ...
        if not display_df.empty:
            cols = [c for c in ['district_name', 'avg_depth_meters', 'extraction_pct', 'recharge_rate_mcm', 
                               'assessment_year', 'stress_level'] if c in display_df.columns]
            st.dataframe(display_df[cols] if cols else display_df, use_container_width=True, hide_index=True)
            st.download_button("📥 Download CSV", display_df.to_csv(index=False).encode('utf-8'), 
                              f"groundwater_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
        ...
        if not display_df.empty:
            ...
            st.dataframe(display_df[cols] if cols else display_df, use_container_width=True, hide_index=True)
            st.download_button("📥 Download CSV", display_df.to_csv(index=False).encode('utf-8'), 
                              f"rainfall_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
        ...
        if not display_df.empty:
            cols = [c for c in ['source_name', 'source_type', 'sector', 'sub_sector', 'consumer_name', 
                               'consumption_mcm', 'record_year', 'season', 'state', 'district'] if c in display_df.columns]
            st.dataframe(display_df[cols] if cols else display_df, use_container_width=True, hide_index=True)
            st.download_button("📥 Download CSV", display_df.to_csv(index=False).encode('utf-8'), 
                              f"usage_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
        ...
        if not display_df.empty:
            ...
            st.dataframe(display_df[cols] if cols else display_df, use_container_width=True, hide_index=True)
            st.download_button("📥 Download CSV", display_df.to_csv(index=False).encode('utf-8'), 
                              f"regional_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
...
if st.sidebar.button("📦 Export All Filtered Data", use_container_width=True):
    export_data = {
        ...
        'rainfall': rainfall[rainfall['district_name'].isin(filtered_sources['district'].unique())] if not filtered_sources.empty and not rainfall.empty else pd.DataFrame(),
